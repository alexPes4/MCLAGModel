...
export default function DevicesTableForNEVMServer(props){
    ...
    useEffect(  ()=>  {
            fetch("http://localhost:8080/objects/NE/getAssignDevicesOnNEVMServer?neId="+neId)
                .then(res=>res.json())
                .then((devices) => {
                    setDevices(setRows(devices,12));
                });
            fetch("http://localhost:8080/objects/NE/getAssignSharedDevicesOnNEVMServer?neId="+neId)
                .then(res=>res.json())
                .then((devices) => {
                    setSharedDevices(setRows(devices,12));
                });
        fetch("http://localhost:8080/objects/NE/getAssignCardsOnNEVMServer?neId="+neId)
            .then(res=>res.json())
            .then((devices) => {
    ...
    return (
        <div style={{  width: '100%' }}>
            ...
            <Stack spacing={2} direction="row" style={{marginBottom: 10}}>
                <AddDevice locId={objectNE.parentId} neId={neId} typeDevice="SharedDevices"></AddDevice>
                <Button variant="outlined" startIcon={<DeleteIcon/>} onClick={ () =>  {
                    releaseDevicesFromNe(neId);
                }}>Release Shared Devices</Button>
            </Stack>
            <DataGrid
                rows={sharedDevices}
                columns={columns}
            ...
            <Stack spacing={2} direction="row" style={{marginBottom: 10}}>
                <AddDevice locId={objectNE.parentId} neId={neId} typeDevice="Devices"></AddDevice>
                <Button variant="outlined" startIcon={<DeleteIcon/>} onClick={ () =>  {
                    releaseDevicesFromNe(neId);
            ...
            <Stack spacing={2} direction="row" style={{marginBottom: 10}} >
                <AddDevice locId={objectNE.parentId} neId={neId} typeDevice="Cards"></AddDevice>
                <Button variant="outlined" startIcon={<DeleteIcon/>} onClick={ () =>  {
                    releaseDevicesFromNe(neId);
...
async function getDevicesToAssign(locId){
    let data=[];
    await fetch("http://localhost:8080/objects/NE/getDevicesToAssignOnVMServer?locId="+locId)
        .then(res=>res.json())
        .then((locs) => {
...
async function getCardsToAssign(locId){
    let data=[];
    await fetch("http://localhost:8080/objects/NE/getCardsToAssignOnVMServer?locId="+locId)
        .then(res=>res.json())
        .then((locs) => {
            data= setRows(locs,null);
            console.log("data:"+data);
...
async function getSharedDevicesToAssign(locId){
    let data=[];
    await fetch("http://localhost:8080/objects/NE/getSharedDevicesToAssignOnVMServer?locId="+locId)
        .then(res=>res.json())
        .then((locs) => {
            data= setRows(locs,null);
            console.log("data:"+data);
...
function  AddDevice(props) {
    ...
    const typeDevice = props.typeDevice;
    const locId=props.locId;
    const neId=props.neId;
    const [open, setOpen] = React.useState(false);
    const handleClickOpen = async () => {
        let data = [];
        if (typeDevice=="SharedDevices") data = await getSharedDevicesToAssign(locId);
        if (typeDevice=="Devices") data = await getDevicesToAssign(locId);
        if (typeDevice=="Cards") data = await getCardsToAssign(locId);
        console.log(data)
        names = data;
        setOpen(true);
    ...
    return (
        <div >
            <Button variant="outlined" startIcon={<AddCircleIcon/>}  onClick={handleClickOpen}>
                Assign {(typeDevice=="Devices")?'Devices':(typeDevice=="SharedDevices")?'Shared Devices':(typeDevice=="Cards")?'Cards':''}
            </Button>
            <Dialog open={open} onClose={handleClose} component="form" action='http://localhost:8080/objects/NE/assigneDevicesToNE' method='POST' >
                <DialogTitle>Add {(typeDevice=="Devices")?'Devices':(typeDevice=="SharedDevices")?'Shared Devices':(typeDevice=="Cards")?'Cards':''}</DialogTitle>
                <DialogContent  >
                    <DialogContentText style={{marginBottom: 20,width:550 }}>
                        Here you can add {(typeDevice=="Devices")?'Devices':(typeDevice=="SharedDevices")?'Shared Devices':(typeDevice=="Cards")?'Cards':''} for the VM Server
                    </DialogContentText>
                    <InputLabel id="demo-multiple-chip-label">Select a {(typeDevice=="Devices")?'Devices':(typeDevice=="SharedDevices")?'Shared Devices':(typeDevice=="Cards")?'Cards':''} from available:</InputLabel>
